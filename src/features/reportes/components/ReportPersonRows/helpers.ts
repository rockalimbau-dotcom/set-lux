 ...
 */
export const useTranslateConcept = () => {
  const { t } = useTranslation();
  return (concepto: string): string => {
 ...
 */
export const useTranslateDietItem = () => {
  const { t } = useTranslation();
...
export const calculateTotal = (
  ...
  extractNumericValue: (val: string) => number,
  formatHorasExtraDecimal: (total: number) => string
