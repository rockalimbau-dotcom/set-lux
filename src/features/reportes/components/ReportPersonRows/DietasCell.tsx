...
const DietasCell: React.FC<DietasCellProps> = ({
  ...
  const isRestCell = cellClasses.includes('report-rest-cell');
  const restBorder = theme === 'light' ? '#94A3B8' : '#CBD5E1';

  return (
    <Td
      key={`${pKey}_${concepto}_${fecha}`}
      className={`text-center report-dietas-cell ${cellClasses} ${hasValue && !isRestCell ? 'report-filled-cell' : ''}`}
      align='center'
      style={isRestCell ? undefined : dayStyle}
    >
        ...
        <div className='w-full relative' ref={dropdownRef}>
          <button
            ...
            style={{
              ...
              borderStyle: 'solid',
              borderColor: isRestCell
                ? restBorder
                : dropdownState.isButtonHovered && theme === 'light'
                  ? '#0476D9'
