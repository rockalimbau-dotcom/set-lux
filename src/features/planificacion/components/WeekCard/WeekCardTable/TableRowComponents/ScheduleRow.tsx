  ...
  return (
    <Row label={t('planning.schedule')}>
      {week.days.map((day: AnyRecord, i: number) => (
        <Td key={i} align='middle' className='overflow-hidden max-w-0'>
          <div className='flex gap-1 sm:gap-1.5 md:gap-2 justify-center w-full'>
            <div className='flex-1 relative max-w-[45%] sm:max-w-none'>
              <input
                type='time'
                value={day.start || ''}
                onChange={e =>
                ...
                placeholder='--:--'
                className={`w-full px-0.5 py-0.5 sm:px-1 sm:py-1 md:px-2 md:py-1 rounded sm:rounded-md md:rounded-lg bg-black/40 border border-neutral-border focus:outline-none focus:ring-1 focus:ring-brand text-left text-[8px] sm:text-[10px] md:text-xs ${
              ...
              />
              {!isValidTime(day.start) && (
                <div className='absolute inset-0 flex items-center px-0.5 py-0.5 sm:px-1 sm:py-1 md:px-2 md:py-1 pointer-events-none text-[8px] sm:text-[10px] md:text-xs text-zinc-400'>
                  --:--
                </div>
              )}
            </div>
