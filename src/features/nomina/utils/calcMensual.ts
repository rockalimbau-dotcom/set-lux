...
export function makeRolePrices(project: any) {
  ...
  if ((import.meta as any).env.DEV) {
    console.debug('[NOMINA.MENSUAL] makeRolePrices - model:', model);
    console.debug('[NOMINA.MENSUAL] makeRolePrices - priceRows:', priceRows);
    console.debug('[NOMINA.MENSUAL] makeRolePrices - params:', p);
  }
  ...
  const getNumField = (row: any, candidates: string[]): number => {
    ...
    if ((import.meta as any).env.DEV) {
      console.debug('[NOMINA.MENSUAL] getNumField - candidates:', candidates);
      console.debug('[NOMINA.MENSUAL] getNumField - row keys:', Object.keys(row));
      console.debug('[NOMINA.MENSUAL] getNumField - row:', row);
    }
    
    for (const key of candidates) {
      const direct = row[key];
      if ((import.meta as any).env.DEV) {
        console.debug(`[NOMINA.MENSUAL] getNumField - trying direct key "${key}":`, direct);
      }
    ...
    for (const key of candidates) {
      ...
      if ((import.meta as any).env.DEV) {
        console.debug(`[NOMINA.MENSUAL] getNumField - trying lowercase "${key.toLowerCase()}":`, v);
      }
  ...
  const getForRole = (roleCode: string, baseRoleCode: string | null = null) => {
    ...
    if ((import.meta as any).env.DEV) {
      console.debug('[NOMINA.MENSUAL] getForRole - roleCode:', roleCode, 'baseRoleCode:', baseRoleCode);
      console.debug('[NOMINA.MENSUAL] getForRole - normalized:', normalized, 'baseNorm:', baseNorm);
    }
    ...
    if ((import.meta as any).env.DEV) {
      console.debug('[NOMINA.MENSUAL] getForRole - row for', normalized, '=> key:', pickedRow.key, row);
      console.debug('[NOMINA.MENSUAL] getForRole - baseRow for', baseNorm, '=> key:', pickedBase.key, baseRow);
      console.debug('[NOMINA.MENSUAL] getForRole - elecRow => key:', pickedElec.key, elecRow);
    }
    ...
    if ((import.meta as any).env.DEV) {
      console.debug('[NOMINA.MENSUAL] getForRole - result for', roleCode, ':', result);
    }
...
const storageKeyVariants = (storageKey: string): string[] => {
  ...
  // Debug temporal
  if (isDev()) {
    console.debug('[NOMINA.MENSUAL.SKV]', storageKey, '=>', result.length, 'variants:', result);
  }
  return result;
};
...
const getCellValueCandidates = (
  ...
  if (isDev()) {
    console.debug('[NOMINA.MENSUAL.GCVC]', 'Looking for', colNames, 'on', iso, 'in keys:', storageKeys);
  }
  ...
  for (const sk of sortedKeys) {
    ...
    if (!cols) {
      if (isDev()) console.debug('[NOMINA.MENSUAL.GCVC]', 'No data for key:', sk);
      continue;
    }
    if (isDev()) {
      console.debug('[NOMINA.MENSUAL.GCVC]', 'Found data for key:', sk, 'columns:', Object.keys(cols));
    }
    // 1) Directo
    for (const cn of colNames) {
      const v = cols?.[cn]?.[iso];
      if (v != null && v !== '') {
        if (isDev()) console.debug('[NOMINA.MENSUAL.GCVC]', 'Found direct match:', sk, cn, iso, '=', v);
        return v;
      }
    }
    ...
    for (const cn of colNames) {
      ...
      if (real) {
        ...
        if (v != null && v !== '') {
          if (isDev()) console.debug('[NOMINA.MENSUAL.GCVC]', 'Found normalized match:', sk, real, iso, '=', v);
          return v;
        }
      }
    }
  }
  if (isDev()) console.debug('[NOMINA.MENSUAL.GCVC]', 'No match found for', colNames, 'on', iso);
  return undefined;
};
...
const dbgLog = (...args: any[]) => {
  if (isDev()) {
    ...
    console.debug('[NOMINA.MENSUAL.AGG]', ...args);
  }
};
...
export function aggregateReports(project: any, weeks: any[], filterISO: ((iso: string) => boolean) | null = null) {
  ...
  for (const w of weeks) {
    ...
    if (isDev()) {
      console.log(`[NOMINA.MENSUAL.DEBUG] Semana ${weekKey}:`, data);
      console.log(`[NOMINA.MENSUAL.DEBUG] Claves disponibles:`, Object.keys(data || {}));
      
      // Buscar específicamente datos para "cccc"
      const ccccKeys = Object.keys(data || {}).filter(k => k.includes('cccc'));
      console.log(`[NOMINA.MENSUAL.DEBUG] Claves que contienen "cccc":`, ccccKeys);
      
      for (const key of ccccKeys) {
        console.log(`[NOMINA.MENSUAL.DEBUG] Datos para ${key}:`, data[key]);
        if (data[key]) {
          for (const col of Object.keys(data[key])) {
            console.log(`[NOMINA.MENSUAL.DEBUG] Columna ${col} en ${key}:`, data[key][col]);
          }
        }
      }
    }
